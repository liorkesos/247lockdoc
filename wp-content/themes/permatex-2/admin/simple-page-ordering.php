...
Modified by Web factory Ltd, 2013
 */
...
class Simple_Page_Ordering {
    ...
    $sortable = ($post_type === 'page' && $sortable);
	...
	public function wp() {
		if ( 0 === strpos( get_query_var('orderby'), 'menu_order' ) ) {	// we can only sort if we're organized by menu order
			...
			wp_enqueue_script( 'simple-page-ordering', get_template_directory_uri() . '/admin/js/' . $script_name, array('jquery-ui-sortable'), '2.1', true );
			wp_enqueue_style( 'simple-page-ordering', get_template_directory_uri() . '/admin/css/simple-page-ordering.css' );
		}
	}
